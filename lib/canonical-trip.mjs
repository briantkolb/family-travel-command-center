...
function validateOptionalStringRecord(container, key, path) {
  if (!Object.hasOwn(container, key)) return {};
  const record = requirePlainObject(container[key], `${path}.${key}`);
  for (const [recordKey, value] of Object.entries(record)) {
    if (typeof value !== "string") {
      throw new Error(`${path}.${key}.${recordKey} must be a string.`);
    }
  }
  return record;
}
...
export function validateCanonicalTrip(trip) {
  ...
  dailyPlan.forEach((day, dayIndex) => {
    ...
    if (Object.hasOwn(day, "tone")) {
      requireNonEmptyString(day.tone, `daily_plan[${dayIndex}].tone`);
    }
  ...
  validateOptionalStringRecord(trip, "airports", "trip");
  validateOptionalStringRecord(trip, "onward_steps", "trip");

  const vaultGroups = validateOptionalRecordArray(
    trip,
    "demo_vault_groups",
    "trip",
  );
  vaultGroups.forEach((group, groupIndex) => {
    const items = validateOptionalRecordArray(
      group,
      "items",
      `demo_vault_groups[${groupIndex}]`,
    );
    items.forEach((item, itemIndex) => {
      requireNonEmptyString(
        item.label,
        `demo_vault_groups[${groupIndex}].items[${itemIndex}].label`,
      );
      requireNonEmptyString(
        item.value,
        `demo_vault_groups[${groupIndex}].items[${itemIndex}].value`,
      );
    });
  });
